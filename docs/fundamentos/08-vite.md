...
VITE_API_URL=https://api.mi-proyecto.com
...
Aunque es un tema avanzado, es bueno que sepas que Vite tiene un "hermano" llamado **Vitest**. Sirve para escribir pruebas automáticas que verifiquen que tus funciones (como la que calcula los puntos de fidelidad de un cliente) no fallen en el futuro.

```typescript
// Ejemplo de un test simple con Vitest
import { expect, test } from 'vitest';
import { sumarPuntos } from './logica-puntos';

test('debe sumar 3 puntos si la compra fue exitosa', () => {
  expect(sumarPuntos(10, 'exito')).toBe(13);
});
```
...
1.  **Limpia el `main.ts`**: Borra el código de ejemplo de Vite y deja solo tu propia lógica (o el ejemplo de Pokémon que hiciste).
